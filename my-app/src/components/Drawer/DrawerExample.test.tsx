...
it('Testing Drawer Open Button', () => {
    ...
    const open = screen.getByTestId('drawer-button');
...
it('Testing imageData Upadate Api', async () => {
    ...
    const res = await axios.put(`${BaseUrl}/imageVideo/${id}`, { stage: 'published' })

    expect(mockData.data.data.title).toEqual(res.data.data.title)
   
})

it('Testing formData Upadate Api', async () => {
    const id = '123';
...
it('Testing Publish Button',async()=>{
    render(<MemoryRouter><DrawerExample data={{
        videoType: undefined,
        _id: "",
        imageFile: [],
